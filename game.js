...
var Bird = function(brain) {
  this.x = 80;
  this.y = 250;
  this.width = 40;
  this.height = 30;
  this.brain = brain ? brain.mutate(0.1) : new NeuralNetwork(5, 8, 2);
...
Bird.prototype.act = function(inputs) {
...
Game.prototype.start = function() {
  ...
  for (var i in Array(15).fill(null)) {
    var b = new Bird(
      null,
      this.bestBrain ? this.bestBrain.copy() : new NeuralNetwork(5, 8, 2)
    );
    this.birds.push(b);
  }
  this.generation++;
...
Game.prototype.update = function() {
  ...
  if (this.birds.length > 0) {
    for (var i = 0; i < this.pipes.length; i += 2) {
      if (this.pipes[i].x + this.pipes[i].width > this.birds[0].x) {
        ...
        nextPipeDistance =
          (this.pipes[i].x - this.birds[0].x) / (this.width - 80);
        break;
      }
    }
  }
  ...
  for (var i in this.birds) {
    if (this.birds[i].alive) {
      if (nextPipe) {
        const inputs = [];
        inputs[0] = this.birds[i].y / this.height;
        inputs[1] = nextPipe.height - this.birds[i].y;
        inputs[2] =
          nextPipe.height + 120 - this.birds[i].y + this.birds[i].height;
        inputs[3] = nextPipeDistance / this.width;
        inputs[4] = this.birds[i].gravity;
        this.birds[i].act(inputs);
      }
      ...
      if (this.birds[i].isDead(this.height, this.pipes)) {
        ...
        if (this.isItEnd()) {
          if (this.score >= this.maxScore) {
            this.bestBrain = this.birds[i].brain;
            console.log("This is the best brain", this.bestBrain);
          }
          this.start();
        }
      }
...
Game.prototype.display = function() {
  ...
  this.ctx.fillText(
    "Alive : " + this.birds.filter(bird => bird.alive).length,
    10,
    100
  );
